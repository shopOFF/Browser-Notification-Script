//////////////////////  working fine
var statsHtmlElement = document.getElementsByClassName("ipo-MainMarkets ")[1];

var observer = new MutationObserver(function (mutations) {
    mutations.forEach(function (mutation) { // <- It always detects changes
        console.log('Dangerous attack!!!');
...
observer.observe(statsHtmlElement, config);
//observer.disconnect();
////////////////////
...
var observer = new MutationObserver(function (mutations) {
    mutations.forEach(function (mutation) {
        console.log('Dangerous attack!!!'); // <- It always detects changes
...
//var elementToObserve = document.getElementsByClassName("ipo-CompetitionRenderer ");
...
for (let i = 0; i < statsHtmlElement.length; i++) {
    ...
    for (let y = 0; y < statsHtmlElement[i].childNodes.length - 1; y++) {
        ...
        statsHtmlElement[i].childNodes[y].childNodes.forEach(el => {
            //  console.log(el);
            if (el.childNodes[1] != undefined) {
                if (Number.parseFloat(el.childNodes[1].innerText) >= 26.00) {
                    //statsHtmlElement[i].childNodes[y].scrollIntoView({block: "end"});
                    el.scrollIntoView({ behavior: "smooth", block: "end", inline: "nearest"});
                    el.style = "background-color: red !important;"
